...
#For Testing - #Get-ADObject -LDAPFilter "(objectClass=computer)" | where-object { $_.name -like "*DEV*" } | select-object -expandproperty name -outvariable comps > $null

#Get a list of all domain computers. For each, get memory statistics for target process (processToMeasure) and add to results container or if error, error container
...
if($utilizationThreshold) {
    ...
    if($utilizationThresholdExceeded.Count -gt 0) {
        ...
        if($unprocessedFlagCount -gt 0) {
            ...
            #For Testing - #Send-MailMessage -From PCNSMS04@wmgpcn.local -To ben.j.fridkis@p66.com -Subject "$processToMeasure Memory Utilization Threshold Exceedance Report-$timestamp"  `
    ...
    if($cleared.Count -gt 0) {
        ...
        #For Testing - Send-MailMessage -From PCNSMS04@wmgpcn.local -To ben.j.fridkis@p66.com -Subject "$processToMeasure Memory Utilization Threshold Exceedances CLEARED-$timestamp"  `
